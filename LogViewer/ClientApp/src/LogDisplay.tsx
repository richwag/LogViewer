...
function LogDisplay({
    ...
    logs: Array<ApplicationLog>;
    setHostContains: { (host: string): void };
    setUserContains: { (user: string): void };
}) {
    ...
    function messageTypeFormatter(d: number) {
        try {
            var result = messageTypes.find(
                ({ MessageTypeId }) => MessageTypeId === d
            );
            ...
            <div>
                <div className="d-flex align-items-center">
                    ...
                    <span
                        ...
                        onClick={() => setUserContains(row.CurrentUserName)}
                    >
                        {row.CurrentUserName}
                    </span>
                </div>
                <div className="d-flex align-items-center">
                    <label className="font-weight-bold mr-1  mb-0">
                        Host:{" "}
                    </label>
                    <span
                        className="btn btn-link"
                        onClick={() => setHostContains(row.Host)}
                    >
                        {row.Host}
                    </span>
                </div>
                <div>
                    ...
                    <SyntaxHighlighter language="javascript" style={docco}>
                        {row.FullErrorJson
                            ? JSON.stringify(row.FullErrorJson, null, 4)
                            : ""}
                    </SyntaxHighlighter>
                </div>
            </div>
